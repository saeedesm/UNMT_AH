...
# Train model
python train.py \
    -train_src_mono  $SRC_VALID.true \
    -train_tgt_mono  $TGT_VALID.true \
    -train_src_bi parallel.tok.clean.tc.ar \
    -train_tgt_bi parallel.tok.clean.tc.he \
    -layers $layers \
    -rnn_size $rnn_size \
    -src_vocab_size 40000 \
    -tgt_vocab_size 40000 \
    -print_every 100 \
    -src_embeddings $ALIGNED_EMBEDDINGS_SRC \
    -tgt_embeddings $ALIGNED_EMBEDDINGS_TGT \
    -discriminator_hidden_size 1024 \
    -supervised_epochs $epochs \
    -unsupervised_epochs 1 \
    -n_unsupervised_batches $unsupervised_batches\
    -sv_load_from model_supervised.pt

python3 translate.py \
    -lang src \
    -model model_supervised.pt \
    -input $SRC_TEST.true \
    -output $TRAIN_DIR/test.$TGT.hyp.true \

echo "Detruecasing hypothesis..."
$DETRUECASER < $SRC_TEST.true > $TRAIN_DIR/test.$TGT.hyp.tok
